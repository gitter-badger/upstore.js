...
window.UPSTORE = {
  ...
  /**
   * Keep track of different UPSTORE bindings
   */
  bindings: {},
  ...
  {
    var i, search = document.querySelectorAll('[upstore-init="'+appId+'"][upstore-arr-key="'+arrKey+'"] [upstore-bind="'+key+'"]');
    for(i = 0;i<search.length;i++)
    {
      search[i].innerHTML = value;
    }

    // search = document.querySelectorAll('[upstore-init="'+appId+'"][upstore-arr-key="'+arrKey+'"] [upstore-bind="'+key+'"]');
  },
}
